...
def calc_binary(keys, bboxes, score, gt_path="./gt_binary_HICO_DET.pkl"):
    hit = []
    idx = np.argsort(score)[::-1]
    gt_bbox = pickle.load(open(gt_path, 'rb'))
    npos = 0
    used = {}
...
def Generate_HICO_detection(db):
    keys, scores, bboxes = [], [], []
    for key in db.keys():
        info = db[key]
        for i in range(len(info['pair_ids'])):
            keys.append(key)
            scores.append(info['obj_scores'][info['pair_ids'][i, 1]])
            hbox = info['boxes'][info['pair_ids'][i, 0]].reshape(-1)
            obox = info['boxes'][info['pair_ids'][i, 1]].reshape(-1)
            bboxes.append(np.concatenate([hbox, obox]))
    ap, rec = calc_binary(keys, bboxes, scores)
    return ap, rec