...
class HICOEvaluator():
    ...
    def compute_map(self):
        ...
        return {'mAP': m_ap, 'mAP rare': m_ap_rare, 'mAP non-rare': m_ap_non_rare, 'mean max recall': m_max_recall, 'AP_full': ap}
...
class HICOEvaluatorBox():
    def __init__(self, preds, gts):
        ...
        for img_preds in preds:
            img_preds = {k: v.to('cpu').numpy() for k, v in img_preds.items()}
            bboxes = [{'bbox': bbox, 'category_id': label, 'score': score, 'id': idx} for idx, (bbox, label, score) in enumerate(zip(img_preds['boxes'], img_preds['labels'], img_preds['scores']))]
        ...
        for img_gts in gts:
            ...
            for hoi in self.gts[-1]['annotations']:
                obj_cat = hoi['category_id']

                if obj_cat not in self.gt_objs:
                    self.gt_objs.append(obj_cat)

                self.sum_gts[obj_cat] += 1
    ...
    def evaluate(self):
        for img_preds, img_gts in zip(self.preds, self.gts):
            ...
            if len(gt_bboxes) != 0:
                bbox_pairs, bbox_overlaps = self.compute_iou_mat(gt_bboxes, pred_bboxes)
                self.compute_fptp(gt_bboxes, bbox_pairs, pred_bboxes, bbox_overlaps)
            else:
                for pred_box in pred_bboxes:
                    obj_cat = pred_box['category_id']
                    if obj_cat not in self.gt_objs:
                        continue
                    self.tp[obj_cat].append(0)
                    self.fp[obj_cat].append(1)
                    self.score[obj_cat].append(pred_box['score'])
        map = self.compute_map()
        return map
    ...
    def compute_fptp(self, gt_bboxes, match_pairs, pred_bboxes, bbox_overlaps):
        pos_pred_ids = match_pairs.keys()
        vis_tag = np.zeros(len(gt_bboxes))
        pred_bboxes.sort(key=lambda k: (k.get('score', 0)), reverse=True)
        if len(pred_bboxes) != 0:
            for pred_bbox in pred_bboxes:
                is_match = 0
                if pred_bbox['category_id'] not in self.gt_objs:
                    continue
                obj_cat = pred_bbox['category_id']
                if len(match_pairs) != 0 and pred_bbox['id'] in match_pairs:
                    max_gt = match_pairs[pred_bbox['id']][np.argmax(bbox_overlaps[pred_bbox['id']])]
                    is_match = 1
                if is_match and vis_tag[max_gt] == 0:
                    self.tp[obj_cat].append(1)
                    self.fp[obj_cat].append(0)
                    vis_tag[max_gt] = 1
                else:
                    self.tp[obj_cat].append(0)
                    self.fp[obj_cat].append(1)
                self.score[obj_cat].append(pred_bbox['score'])
